...
public class Alarm {
 ...
 public static void selfTest() {
    System.out.println("Alarm wake order and differing times test");
...
KThread t3 = new KThread(new TestThread("Thread-3000", 3000));
    ...
    System.out.println("Alarm x<=0 test");

KThread t4 = new KThread(new Runnable() {
    public void run() {
        long start = Machine.timer().getTime();
        System.out.println("Thread-zero sleeping at " + start);

        alarm.waitUntil(0);

        long end = Machine.timer().getTime();
        System.out.println("Thread-zero woke at " + end +
                           " (should be immediate, diff=" + (end - start) + ")");
    }
});
...
KThread t5 = new KThread(new Runnable() {
    public void run() {
        long start = Machine.timer().getTime();
        System.out.println("Thread-negative sleeping at " + start);

        alarm.waitUntil(-500);

        long end = Machine.timer().getTime();
        System.out.println("Thread-negative woke at " + end +
                           " (should be immediate, diff=" + (end - start) + ")");
    }
});
...
System.out.println("Alarm stress test");

final int numThreads = 20;

for (int i = 0; i < numThreads; i++) {
    final int id = i;
    final long wait = (long)(Math.random() * 5000);

    new KThread(new Runnable() {
        public void run() {
            long start = Machine.timer().getTime();
            System.out.println("Stress-" + id + " sleeping for " + wait);

            alarm.waitUntil(wait);

            long end = Machine.timer().getTime();
            System.out.println("Stress-" + id + " woke after " +
                               (end - start) + " ticks");
        }
    }).fork();
}

// Give enough time for all to finish
